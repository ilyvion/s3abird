...
const copyImg = () => {
    return gulp.src("./static/img/**/*").pipe(gulp.dest(`${OUTPUT}/img`));
};

const copyFavicon = () => {
    return gulp.src("./static/favicon/**/*").pipe(gulp.dest(`${OUTPUT}`));
};

const copyJs = () => {
    return gulp.src("./static/**/*.js").pipe(gulp.dest(OUTPUT));
};
...
exports.default = series(parallel(copy, sass, js), (done) => {
    done();
    process.exit(0); // Force exit when all done
});
